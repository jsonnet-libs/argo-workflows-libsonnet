...
### fn withTaskResultsCompleted

```ts
withTaskResultsCompleted(taskResultsCompleted)
```

"Have task results been completed? (mapped by Pod name) used to prevent premature garbage collection of artifacts."

### fn withTaskResultsCompletedMixin

```ts
withTaskResultsCompletedMixin(taskResultsCompleted)
```
...
## obj storedWorkflowTemplateSpec.podGC.deleteDelayDuration

"Duration is a wrapper around time.Duration which supports correct\nmarshaling to YAML and JSON. In particular, it marshals into strings, which\ncan be used as map keys in json."

### fn storedWorkflowTemplateSpec.podGC.deleteDelayDuration.withDuration

{
  ...
  '#withTasks':: d.fn(help='"Tasks are a list of DAG tasks MaxItems is an artificial limit to limit CEL validation costs - see note at top of file"', args=[d.arg(name='tasks', type=d.T.array)]),
  withTasks(tasks): { tasks: if std.isArray(v=tasks) then tasks else [tasks] },
  '#withTasksMixin':: d.fn(help='"Tasks are a list of DAG tasks MaxItems is an artificial limit to limit CEL validation costs - see note at top of file"\n\n**Note:** This function appends passed data to existing values', args=[d.arg(name='tasks', type=d.T.array)]),
